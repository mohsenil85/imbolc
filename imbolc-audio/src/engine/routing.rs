...
use super::{
    TrackNodes, GROUP_BUS_PROCESSING, GROUP_MASTER, GROUP_OUTPUT, GROUP_PROCESSING, GROUP_SOURCES,
    VST_UGEN_INDEX,
};
use imbolc_types::{
...
impl AudioEngine {
    ...
    fn build_instrument_chain(
        ...
        let channels = instrument.channel_strip.channel_config.channels();
        ...
        if instrument.source.is_audio_input() {
            ...
            for p in &instrument.source_params {
                ...
                let val = if p.name == "gain" && !instrument.channel_strip.active {
                    0.0
                } else {
                    val
                };
        ...
        let output_node_id = {
            ...
            let mute = if any_solo {
                ...
                instrument.channel_strip.mute || session.mixer.master_mute
            };
            ...
            let output_bus = if let Some(group_id) = instrument.layer.group {
                ...
                match instrument.channel_strip.output_target {
            ...
            let params = vec![
                ...
                (
                    "level".to_string(),
                    instrument.channel_strip.level * session.mixer.master_level,
                ),
                ("mute".to_string(), if mute { 1.0 } else { 0.0 }),
                ("pan".to_string(), instrument.channel_strip.pan),
    ...
    fn build_bus_effect_chain(
        ...
        for effect in bus.channel_strip.effects() {
            if !effect.enabled {
                continue;
            }
            let node_id = self.next_node_id;
    ...
    fn build_layer_group_effect_chain(
        ...
        for effect in gm.channel_strip.effects() {
            if !effect.enabled {
                continue;
            }
            let node_id = self.next_node_id;
    ...
    pub fn rebuild_instrument_routing(
        ...
        if let Some(ref client) = self.backend {
            ...
            for &node_id in self.bus_eq_node_map.values() {
                let _ = client.free_node(node_id);
            }
        ...
        for group_mixer in &session.mixer.layer_group_mixers {
            if let Some(&group_bus) = self.layer_group_audio_buses.get(&group_mixer.group_id) {
                ...
                let group_out = match group_mixer.channel_strip.output_target {
                    imbolc_types::OutputTarget::Bus(id) => {
                        self.bus_audio_buses.get(&id).copied().unwrap_or(0) as f32
                    }
                    imbolc_types::OutputTarget::Master => 0.0,
                ...
                let params = vec![
                    ...
                    ("level".to_string(), group_mixer.channel_strip.level),
                    ("mute".to_string(), if mute { 1.0 } else { 0.0 }),
                    ("pan".to_string(), group_mixer.channel_strip.pan),
                ];
                if let Some(ref client) = self.backend {
                    client
                ...
                for send in group_mixer.channel_strip.sends.values() {
                    if !send.enabled || send.level <= 0.0 {
                        continue;
                    }
                    if let Some(&bus_audio) = self.bus_audio_buses.get(&send.bus_id) {
        ...
        for bus in &session.mixer.buses {
            if let Some(&bus_audio) = self.bus_audio_buses.get(&bus.id) {
                ...
                let params = vec![
                    ("in".to_string(), post_effect_bus as f32),
                    ("level".to_string(), bus.channel_strip.level),
                    ("mute".to_string(), if mute { 1.0 } else { 0.0 }),
                    ("pan".to_string(), bus.channel_strip.pan),
                ];
                if let Some(ref client) = self.backend {
                    client
                        .create_synth("imbolc_bus_out", node_id, GROUP_BUS_PROCESSING, &params)
    ...
    pub fn delete_instrument_routing(&mut self, instrument_id: TrackId) -> Result<(), String> {
        ...
        self.bus_allocator.free_instrument_buses(instrument_id);
    ...
    pub fn rebuild_bus_processing(
        ...
        // Rebuild master effect chain + EQ (in GROUP_MASTER, after bus processing)
        self.build_master_effect_chain(session)?;
        ...
        for instrument in &state.tracks {
            if let Some(nodes) = self.node_map.get(&instrument.id) {
                ...
                client
                    ...
                    .map_err(|e| e.to_string())?;
                client
                    .set_param(nodes.output, "pan", instrument.channel_strip.pan)
                    .map_err(|e| e.to_string())?;
            }
        }
        Ok(())
    ...
    /// Set a master effect parameter in real-time (targeted /n_set, no rebuild).
    ...
    /// Build the master effect chain + EQ in GROUP_MASTER.
    ///
    ...
    fn build_master_effect_chain(&mut self, session: &SessionState) -> Result<(), String> {
        ...
        for (_, node_id) in self.master_effect_node_map.drain() {
            self.node_registry.unregister(node_id);
            if let Some(client) = self.backend.as_ref() {
                let _ = client.free_node(node_id);
            }
        ...
        if use_inplace {
            ...
            for effect in cs.effects() {
                ...
                let effect_out_bus = self.bus_allocator.get_or_alloc_audio_bus(
                    master_alloc_id,
                    &format!("master_fx_{}_out", effect.id),
                );
                ...
                for p in &effect.params {
                    if let Some((mapped_name, mapped_value)) = self
                        .remap_effect_bus_param_from_state(effect.effect_type, &p.name, &p.value)
                    {
                        params.push((mapped_name.to_string(), mapped_value));
                        continue;
                    }
                ...
                self.node_registry.register(node_id);
                self.master_effect_node_map.insert(effect.id, node_id);
                current_bus = effect_out_bus;
            }
            ...
            // Final stage: replace bus 0 with the processed signal
            let replace_node_id = self.next_node_id;
            self.next_node_id += 1;
            let client = self.backend.as_ref().ok_or("Not connected")?;
            client
                .create_synth(
                ...
                .map_err(|e| e.to_string())?;
            self.node_registry.register(replace_node_id);
