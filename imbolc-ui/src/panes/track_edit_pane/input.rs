...
impl TrackEditPane {
    pub(super) fn handle_action_impl(
        ...
        let ActionId::TrackEdit(action) = action else {
            return Action::None;
        };

        match action {
            // Normal pane actions
            TrackEditActionId::Done => self.emit_update(),
            ...
            TrackEditActionId::ToggleChannelConfig => {
                if let Some(id) = self.instrument_id {
                    ...
                    if let Some(ProcessingStage::Effect(effect)) = self.processing_chain.get(i) {
                        if effect.effect_type.is_vst() {
                            if let Some(instrument_id) = self.instrument_id {
                                Action::Track(TrackAction::ShowVstEffectParams(
                                    instrument_id,
                                    effect.id,
                                ))
                            } else {
