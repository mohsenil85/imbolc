...
mod tests {
    ...
    fn vst_has_no_envelope_rows() {
        let inst = Instrument::new(TrackId::new(1), SourceType::Vst(VstPluginId::new(0)));
        let total = inst.total_editable_rows();
        for i in 0..total {
    ...
    fn timestretch_has_sampler_config_and_correct_params() {
        let inst = Instrument::new(TrackId::new(1), SourceType::TimeStretch);
    ...
    fn sampler_config_mut_works() {
        let mut inst = Instrument::new(TrackId::new(1), SourceType::PitchedSampler);
        assert!(inst.sampler_config_mut().is_some());
        let mut saw = Instrument::new(TrackId::new(2), SourceType::Saw);
    ...
    fn drum_sequencer_some_for_kit() {
        let inst = Instrument::new(TrackId::new(1), SourceType::Kit);
        assert!(inst.drum_sequencer().is_some());
    }

    #[test]
    fn drum_sequencer_mut_works() {
        let mut inst = Instrument::new(TrackId::new(1), SourceType::Kit);
    ...
    fn vst_source_params_returns_values_for_vst() {
        let mut inst = Instrument::new(TrackId::new(1), SourceType::Vst(VstPluginId::new(0)));
